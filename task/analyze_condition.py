...
def adapt_sign_up_time(store_id, relations):
    """
    适配出所有符合注册条件的客户id
    :param store_id: 店铺id
    ...
    :return: 客户ids
    """
    min_time, max_time = date_relation_convert(relations[0]["relation"], relations[0]["values"],
...
def adapt_last_order_created_time(store_id, relations):
    """
...
def adapt_last_opened_email_time(store_id, relations):
    """
...
def adapt_last_click_email_time(store_id, relations):
    """
        适配出所有打开过邮件的时间符合条件的客户ids
        :param store_id: 店铺id
        :param relations: 时间范围条件
...
def adapt_placed_order(store_id, relations):
    """
    ...
    min_time, max_time = date_relation_convert(relations[1]["relation"], relations[1]["values"],
                                               relations[1].get("unit", "days"))
    adapt_customers = order_filter(store_id=store_id, status=0, relation=relations[0]["relation"],
                                   value=relations[0]["values"][0], min_time=min_time, max_time=max_time)
    return adapt_customers


def adapt_paid_order(store_id, relations):
    """
    ...
    min_time, max_time = date_relation_convert(relations[1]["relation"], relations[1]["values"],
                                               relations[1].get("unit", "days"))
    adapt_customers = order_filter(store_id=store_id, status=1, relation=relations[0]["relation"],
                                   value=relations[0]["values"][0], min_time=min_time, max_time=max_time)
    return adapt_customers


def email_opt_filter(store_id, opt_type, relation, value, min_time, max_time):
    """
    筛选满足邮件操作条件的客户的id
    :param store_id: 店铺id
...
def adapt_opened_email(store_id, relations):
    """
    ...
    """
    # relations 两个, 第一个是数量，第二个是时间范围
    min_time, max_time = date_relation_convert(relation=relations[1]["relation"], values=relations[1]["values"],
                                               unit=relations[1].get("unit", "days"))
    customers = email_opt_filter(store_id=store_id, opt_type=0, relation=relations[0]["relation"], value=relations[0]["values"][0],
                     min_time=min_time, max_time=max_time)
    return customers


def adapt_clicked_email(store_id, relations):
    """
    ...
    min_time, max_time = date_relation_convert(relation=relations[1]["relation"], values=relations[1]["values"],
                                               unit=relations[1].get("unit", "days"))
    customers = email_opt_filter(store_id=store_id, opt_type=1, relation=relations[0]["relation"], value=relations[0]["values"][0],
                     min_time=min_time, max_time=max_time)
    return customers


def adapt_last_order_status(store_id, relations):
    """
...
def adapt_is_accept_marketing(store_id, relations):
    """
...
def adapt_customer_email(store_id, relations):
    """
    适配出邮件地址符合条件的客户id
    :param store_id: 店铺id
    :param relations: 筛选条件　
...
def adapt_total_order_amount(store_id, relations):
    """
    适配出总订单金额符合条件的客户id
    :param store_id: 店铺id
    :param relations: 筛选条件　
...
def get_adapt_customers(condition, store_id):
    """
    根据综合条件，筛选出符合的顾客列表
    ...
    try:
        for gc in group_conditions:
            ...
            for child in children:
                ...
                # 这几个的relation是两个
                # if condition_name in ["Customer placed order", "Customer paid order", "Customer opened email",
                #                  "Customer clicked email"]:

                adapter = condition_dict.get(condition_name, None)
                if adapter:
            ...
            logger.info("adapt condition={}, customers={}".format(gc.get("group_name", "unknown"), children_final_customers))
        ...
        logger.exception("get_adapt_customers catch exception: {}".format(e))

    return final_customers


def get_conditions(store_id=None, condition_id=None):
    result = []
    try:
        conn = DBUtil().get_instance()
...
if __name__ == '__main__':
    ...
    for cond in conditions:
        cus = get_adapt_customers(condition=json.loads(cond["relation_info"]), store_id=cond["store_id"])
        print(cus)