...
class ProductRecommend(MongoDBUtil, DBUtil):
    ...
    def generate_product_block(self, product_list):
        """
        生成HTML中产品信息块，
        ...
        for product in product_list:
            html_str += self.format_str.format(**product)
        return html_str
    ...
    def get_card_product_mongo(self, customer_id, length=6):
        ...
        try:
            ...
            # 获取购物车产品ID
            cart_products = db.shopify_unpaid_order.find({"customer.id": customer_id}, {"_id":0, "line_items": 1})
            product_ids = []
            ...
            for product in product_infos:
                if len(products) >= 6:
                    break
                products.append({"product_url": "https://{}.myshopify.com/products/{}".format(product["site_name"],
                                                                                              product["handle"]),
...
if __name__ == '__main__':
    ...
    print(pr.generate_product_block(pr.get_card_product_mongo(326597345317)))